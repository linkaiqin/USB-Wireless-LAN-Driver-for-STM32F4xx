...
int RTMPAllocateNdisPacketCtr = 0;
int RTMPFreeNdisPacketCtr = 0;
...
PNDIS_PACKET ClonePacket(
	...
	IN ULONG DataSize)
{
    DBGPRINT(RT_DEBUG_ERROR, ("%s done error!!\n", __FUNCTION__));    
    while(1);
#if 0    
...
BOOLEAN RtmpOsSemaInitLocked(IN RTMP_OS_SEM *pSem,
			     IN LIST_HEADER *pSemList) {
	if (RTMP_OS_Alloc_RscOnly(pSem, sizeof (OS_SEMA)) == FALSE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: alloc semaphore fail!\n", __FUNCTION__));
		return FALSE;	/* allocate fail */
	}

	OS_SEM_EVENT_INIT_LOCKED((OS_SEMA *) (pSem->pContent));
	return TRUE;
}
...
BOOLEAN RtmpOsSemaInit(IN RTMP_OS_SEM *pSem,
	...
	OS_SEM_EVENT_INIT((OS_SEMA *) (pSem->pContent));
	return TRUE;
}
