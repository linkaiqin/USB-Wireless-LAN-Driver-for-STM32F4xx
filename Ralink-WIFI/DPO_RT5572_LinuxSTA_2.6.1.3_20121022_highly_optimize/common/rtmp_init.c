...
const int rtusb_tx_buf_len = sizeof(HTTX_BUFFER);
const int rtusb_rx_buf_len = MAX_RXBULK_SIZE;
const int rtusb_tx_buf_cnt = 4;
const int rtusb_rx_buf_cnt = RX_RING_SIZE;
	...
	IN	VOID					*pRsc)
{
	...
	else
	{
		...
		DBGPRINT(RT_DEBUG_TRACE/*RT_DEBUG_ERROR*/, ("%s: add timer obj %lx!\n", __FUNCTION__, (ULONG)pRsc));
	}
}
