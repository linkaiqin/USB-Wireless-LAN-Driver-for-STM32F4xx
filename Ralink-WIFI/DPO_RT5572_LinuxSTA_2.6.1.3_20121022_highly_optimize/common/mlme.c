...
#if MLMEQUEUE_ALLOC_INUSE        
        Queue->Entry[i].Msg = NULL;
#else
	...
	IN ULONG Priv) 
{
    ...
    if (Msg != NULL)
	{
        Queue->Entry[Queue->Tail].Msg= kmalloc(MsgLen, GFP_ATOMIC);
        if(!Queue->Entry[Queue->Tail].Msg)
        {
            printk("kmalloc MLME_QUEUE_ELEM->Msg Failed!! size:%d func:%s line:%d\n",\
                                         MsgLen,__FUNCTION__,__LINE__);
