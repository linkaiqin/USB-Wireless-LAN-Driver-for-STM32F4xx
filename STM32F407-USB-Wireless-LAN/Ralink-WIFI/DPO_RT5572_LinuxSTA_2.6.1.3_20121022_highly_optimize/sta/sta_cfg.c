	...
	IN	UINT32					Subcmd)
{
    ...
    #ifdef MLMEQUEUE_ALLOC_INUSE
    pMsgElem->Msg = kmalloc(sizeof(MLME_DEAUTH_REQ_STRUCT) > sizeof(MLME_DISASSOC_REQ_STRUCT)?\
                                     sizeof(MLME_DEAUTH_REQ_STRUCT):sizeof(MLME_DISASSOC_REQ_STRUCT), GFP_ATOMIC);
    if(!pMsgElem->Msg)
    {
        os_free_mem(NULL,pMsgElem);
        printk("kmalloc MLME_QUEUE_ELEM->Msg Failed!! size:%d func:%s line:%s\n",\
                                     sizeof(MLME_DEAUTH_REQ_STRUCT) > sizeof(MLME_DISASSOC_REQ_STRUCT)?\
                                     sizeof(MLME_DEAUTH_REQ_STRUCT):sizeof(MLME_DISASSOC_REQ_STRUCT),__FUNCTION__,__LINE__);
        return NDIS_STATUS_FAILURE;
    }
    #endif
