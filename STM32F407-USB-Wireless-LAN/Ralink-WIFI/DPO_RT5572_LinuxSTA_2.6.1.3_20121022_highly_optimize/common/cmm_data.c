	...
	IN	UINT			Length)
{
    ...
    UCHAR rtmpHwHdr[TXINFO_SIZE + 32];//modified by LKQ in 2015.9.7
    ...
    if(pAd->chipCap.TXWISize > 32)//add by LKQ in 2015.9.7
    {
        printk("MiniportMMRequest pAd->chipCap.TXWISize > 32  hang!\r\n");
        while(1);
    }
	...
	while (DataSize > LENGTH_802_3)
	{
			...
			)
		{
			...
			if (Elem != NULL)
			{
                ...
                Elem->Msg = kmalloc(LENGTH_802_11 + LENGTH_802_1_H + PayloadSize, GFP_ATOMIC);
                if(!Elem->Msg)
                {
                    os_free_mem(NULL,Elem);
                    printk("kmalloc MLME_QUEUE_ELEM->Msg Failed!! size:%d func:%s line:%s\n",\
                                                 LENGTH_802_11 + LENGTH_802_1_H + PayloadSize,__FUNCTION__,__LINE__);
                }
                else
                #endif   
                {
