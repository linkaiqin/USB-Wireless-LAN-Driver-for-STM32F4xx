...
extern OS_TCB USBHProbeTaskTCB;
...
//   OS_ERR err;
  ...
  pdev->host.ConnSts = USBH_DEV_DISCONNECTED;
//  USBH_HCD_INT_fops->DevDisconnected(pdev);
//    OSTaskQPost(&USBHProbeTaskTCB,(void *)USBH_DEV_DISCONNECTED,1,OS_OPT_POST_FIFO,&err);
//    if(err != OS_ERR_NONE)
//        USBH_DBG("USB_OTG_USBH_handle_Disconnect_ISR OSTaskQPost Failed:%d\r\n",err); 
//        
  /* Clear interrupt */
...
//   OS_ERR err;
  ...
  if (hprt0.b.prtconndet)
  {
    ...
    hprt0_dup.b.prtconndet = 1;
     ...
     pdev->host.ConnSts = USBH_DEV_ATTACHED;
//    OSTaskQPost(&USBHProbeTaskTCB,(void *)USBH_DEV_ATTACHED,1,OS_OPT_POST_FIFO,&err);
//    if(err != OS_ERR_NONE)
//        USBH_DBG("USB_OTG_USBH_handle_port_ISR OSTaskQPost Failed:%d\r\n",err);
...
uint32_t USB_OTG_USBH_handle_hc_n_Out_ISR (USB_OTG_CORE_HANDLE *pdev , uint32_t num)
{
  ...
  else if (hcint.b.chhltd)
  {
    ...
    if(num >= HC_MAX)
    {
        USBH_DBG("USB_OTG_USBH_handle_hc_n_In_ISR num error num:%d  HC_MAX:%d\r\n",num,HC_MAX);
        return 1;
    }
    urb = _usb_device.urb[num];
    if(!urb)
    {
        USBH_DBG("USB_OTG_USBH_handle_hc_n_In_ISR urb is NULL\r\n");
        return 1;
    }
    task_req = OSMemGet(&ReqQueueMem,&err);
    if(err != OS_ERR_NONE)
    {
        USBH_DBG("USB_OTG_USBH_handle_hc_n_In_ISR OSMemGet ReqQueueMem Faild %d\r\n",err);
        return 1;
    }
    task_req->req_type = USBH_REQ_TYPE_RROGRESS;
    task_req->req_msg = USBH_RROGRESS_FROM_INT;
    task_req->urb = urb;
    OSTaskQPost(&USBHTaskTCB, task_req, sizeof(struct usbh_task_req), OS_OPT_POST_FIFO, &err);
    if(err != OS_ERR_NONE){
        USBH_DBG("USB_OTG_USBH_handle_hc_n_In_ISR OSTaskQPost Failed %d \r\n",err);
...
uint32_t USB_OTG_USBH_handle_hc_n_In_ISR (USB_OTG_CORE_HANDLE *pdev , uint32_t num)
{
  ...
  else if (hcint.b.xfercompl)
  {
    ...
    BULK_DBG(("USB_OTG_USBH_handle_hc_n_In_ISR   hcint.b.xfercompl num:%d\r\n",num));
  ...
  else if (hcint.b.chhltd)
  {
    ...
    else if(pdev->host.HC_Status[num] == HC_NAK)//new add
    {
    ...
    BULK_DBG(("USB_OTG_USBH_handle_hc_n_In_ISR hcint.b.chhltd HC_Status:%d\r\n",pdev->host.HC_Status[num]));
    ...
    if(err != OS_ERR_NONE)
    {
        USBH_DBG("USB_OTG_USBH_handle_hc_n_In_ISR OSMemGet ReqQueueMem Faild %d  HC_Status:%d\r\n",err,pdev->host.HC_Status[num]);
        return 1;
    }
    task_req->req_type = USBH_REQ_TYPE_RROGRESS;
    task_req->req_msg = USBH_RROGRESS_FROM_INT;    
  ...
  {  
              ...
              (hcchar.b.eptype == EP_TYPE_BULK))
    {
      ...
      BULK_DBG(("USB_OTG_USBH_handle_hc_n_In_ISR   hcint_nak_ctr:%d  num:%d\r\n",hcint_nak_ctr,num));
      
    //  printf("hcint.b.nak ctr:%d retry...\r\n",hcint_nak_ctr);
      if(/*(hcint_nak_ctr >= 1)&&*/(hcchar.b.eptype == EP_TYPE_BULK))
      {
   //     printf("hcint.b.nak USB_OTG_HC_Halt\r\n");
        UNMASK_HOST_INT_CHH (num);
        USB_OTG_HC_Halt(pdev, num);   
      }
      else//if hcchar.b.eptype == EP_TYPE_CTRL, it has a timer to proess timeout.
      {
...
//         printf("GRXSTS_PKTSTS_IN grxsts.b.bcnt:%d\r\n",grxsts.b.bcnt);
      ...
      pdev->host.XferCnt[channelnum]  = count;//  cover see USB_OTG_HC_StartXfer.
