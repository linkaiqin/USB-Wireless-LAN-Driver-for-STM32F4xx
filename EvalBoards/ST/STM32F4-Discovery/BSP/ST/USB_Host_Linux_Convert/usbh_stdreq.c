...
static void USBH_ParseCfgDesc (USBH_CfgDesc_TypeDef* cfg_desc,
                               ...
                               struct usb_host_endpoint   ep_desc[USBH_MAX_NUM_ENDPOINTS],                                                           
                               uint8_t *buf, 
                               uint16_t length);
...
* @retval Status
*/
USBH_Status USBH_Get_DevDesc(struct usb_device *dev,
                             uint8_t length)
{
  
  USBH_Status status;
  
  if((status = USBH_GetDescriptor(dev, 
                                  USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD,                          
                                  USB_DESC_DEVICE, 
                                  dev->USB_OTG_Core.host.Rx_Buffer,
                                  length)) == USBH_OK)
  {
    /* Commands successfully sent and Response Received */    
    
    USBH_ParseDevDesc(&dev->descriptor, dev->USB_OTG_Core.host.Rx_Buffer, length);
  }
  return status;      
}
...
* @retval Status
*/
USBH_Status USBH_Get_CfgDesc(struct usb_device *dev, 
                                    uint16_t length)
{
  USBH_Status status;
//  uint16_t index = 0;
...
//    {
//      USBH_CfgDesc[index] = buff[index];
//    }
    
    /* Commands successfully sent and Response Received  */       
    USBH_ParseCfgDesc ((USBH_CfgDesc_TypeDef*)&dev->config->desc,
                       (USBH_InterfaceDesc_TypeDef*)&dev->intf->altsetting->desc,
                       &dev->ep[1], 
                       dev->USB_OTG_Core.host.Rx_Buffer,
                       length); 
...
* @retval Status
*/
USBH_Status USBH_Get_StringDesc(struct usb_device *dev,
                                ...
                                uint16_t length)
{
  ...
  {
    /* Commands successfully sent and Response Received  */       
    USBH_ParseStringDesc(dev->USB_OTG_Core.host.Rx_Buffer,buff, length);    
  }
  return status;
}
...
* @retval Status
*/
USBH_Status USBH_GetDescriptor(struct usb_device *dev,                              
                               uint8_t  req_type,
                               uint16_t value_idx, 
                               uint8_t* buff, 
                               uint16_t length )
...
//  phost->Control.setup.b.wLength.w = length;   
  ...
  ret = usb_control_msg(dev,usb_sndctrlpipe(dev,0),USB_REQ_GET_DESCRIPTOR,USB_D2H | req_type,value_idx,windex,buff,length,5000);
    if(ret < 0)
        return USBH_FAIL;
    else
        return USBH_OK;
...
* @retval Status
*/
USBH_Status USBH_SetAddress(struct usb_device *dev, 
...
//  phost->Control.setup.b.wLength.w = 0;
  ...
  ret = usb_control_msg(dev, usb_sndctrlpipe(dev,0),USB_REQ_SET_ADDRESS,USB_H2D | USB_REQ_RECIPIENT_DEVICE | \
                                                    USB_REQ_TYPE_STANDARD,(uint16_t)DeviceAddress,0,0,0,5000);
    if(ret < 0)
        return USBH_FAIL;
    else
        return USBH_OK;  
...
* @retval Status
*/
USBH_Status USBH_SetCfg(struct usb_device *dev, 
                        uint16_t cfg_idx)
{
  int ret;
//  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_RECIPIENT_DEVICE |\
...
//  phost->Control.setup.b.wLength.w = 0;           
    ...
    ret = usb_control_msg(dev, usb_sndctrlpipe(dev,0),USB_REQ_SET_CONFIGURATION,USB_H2D | USB_REQ_RECIPIENT_DEVICE |\
                    USB_REQ_TYPE_STANDARD,cfg_idx,0,0,0,5000);
   if(ret < 0)
        return USBH_FAIL;
    else
        return USBH_OK;  
...
* @retval Status
*/
USBH_Status USBH_SetInterface(struct usb_device *dev, 
...
//  phost->Control.setup.b.wLength.w = 0;           
    ret = usb_control_msg(dev,usb_sndctrlpipe(dev,0),USB_REQ_SET_INTERFACE,USB_H2D | USB_REQ_RECIPIENT_INTERFACE | \
                    USB_REQ_TYPE_STANDARD,altSetting,ep_num,0,0,5000);
   if(ret < 0)
        return USBH_FAIL;
    else
        return USBH_OK;   
...
* @retval Status
*/
USBH_Status USBH_ClrFeature(struct usb_device *dev,
                            uint8_t ep_num, 
                            uint8_t hc_num) 
{
  int ret;
...
//  phost->Control.setup.b.wLength.w = 0;           
  ...
  { /* EP Type is IN */
    dev->USB_OTG_Core.host.hc[hc_num].toggle_in = 0; 
  }
  else
  {/* EP Type is OUT */
    dev->USB_OTG_Core.host.hc[hc_num].toggle_out = 0; 
  }

   ret =  usb_control_msg(dev,usb_sndctrlpipe(dev,0),USB_REQ_CLEAR_FEATURE,USB_H2D | 
                                         USB_REQ_RECIPIENT_ENDPOINT |
                                         USB_REQ_TYPE_STANDARD,FEATURE_SELECTOR_ENDPOINT,ep_num,0,0,5000);
   if(ret < 0)
        return USBH_FAIL;
    else
        return USBH_OK;    
        ...
        if (pdesc->bDescriptorType   == USB_DESC_TYPE_INTERFACE) 
        {
          ...
          {          
            while (ep_ix < temp_pif.bNumEndpoints) 
            {
              ...
              {  
                pep               = (USBH_EpDesc_TypeDef *)&ep_desc[ep_ix].desc;
