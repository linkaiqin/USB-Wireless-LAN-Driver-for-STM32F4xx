...
//    int           scanflags = 0;      /* Flags for scan */
    char *   buffer = NULL;      /* Results */
    int           buflen = 1024;//IW_SCAN_MAX_DATA; /* Min for compat WE<17 */
	struct iw_request_info info;// = { .cmd = SIOCSIWSCAN, .flags = 0 };
    ...
    info.cmd = SIOCSIWSCAN;
    info.flags = 0;
    wrq.u.data.pointer = NULL;
    wrq.u.data.flags = 0;
    wrq.u.data.length = 0;
    handler = rt28xx_iw_handler_def.standard[SIOCSIWSCAN - SIOCIWFIRST];
    ret = handler(_pnet_device, &info, &wrq.u, NULL);
 //   ret = rt_ioctl_siwscan(_pnet_device, &info, &wrq.u, NULL);    
    if(ret < 0)
    {
        USBH_DBG("  Interface doesn't support scanning : %d\n\n",ret);
        return(-1);
    }
    

    buffer = kmalloc(buflen ,0);
    if(buffer == NULL)
    {
        USBH_DBG("%s: Allocation failed\r\n", __FUNCTION__);
        return(-1);
    }
    ...
    handler = rt28xx_iw_handler_def.standard[SIOCGIWSCAN - SIOCIWFIRST];
    ret = handler(_pnet_device, &info, &wrq.u, buffer);
  //  ret = rt_ioctl_giwscan(_pnet_device, &info, &wrq.u, buffer);
    if(ret < 0)
    {
      /* Bad error */
      kfree(buffer);
      USBH_DBG("Failed to read scan data : %d\n\n",ret);
      return(-2);
    }
...
find_command(const char *	cmd)
{
  ...
  for(i = 0; iwlist_cmds[i].cmd != NULL; ++i)
    {
      /* No match -> next one */
      if(strncmp(iwlist_cmds[i].cmd, cmd, len) != 0)
	continue;
 ...
 */
#include "shell.h"
CPU_INT16S iwlist_main (CPU_INT16U argc,
                        CPU_CHAR *argv[],
