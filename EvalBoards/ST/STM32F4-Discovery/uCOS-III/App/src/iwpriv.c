...
{
  struct iwreq	wrq;
  u_char	*buffer = NULL;	/* Only that big in v25 and later */
...
//      offset = sizeof(__u32);
    ...
    buffer = kmalloc(IW_PRIV_SIZE_MASK,0);
    if(!buffer)
        return -1;
...
//  }
	...
	handler = get_handler(_pnet_device, priv[k].cmd);
	if (handler) 
        ret = handler(_pnet_device, &info, (union iwreq_data *) iwp, extra);
    else
	    ret = _pnet_device->do_ioctl(_pnet_device, (struct ifreq*)&wrq, priv[k].cmd);
    

  /* Perform the private ioctl */
  if(ret /*ioctl(skfd, priv[k].cmd, &wrq)*/ < 0)
    {
      fprintf(stderr, "Interface doesn't accept private ioctl...\n");
      fprintf(stderr, "%s (%X): %d\n", cmdname, priv[k].cmd, ret);
      kfree(buffer);
      return(-1);
    }
 ...
 */
#include "shell.h"
CPU_INT16S iwpriv_main (CPU_INT16U argc,
                        CPU_CHAR *argv[],
...
//	else
	  ...
	  if(argc == 1)
	    print_priv_info(0, "", NULL, 0);
	  else
	    /* Special cases take two... */
	    /* All */
	    if((!strncmp(argv[1], "-a", 2)) ||
	       (!strcmp(argv[1], "--all")))
	      print_priv_all(0, argv[1], NULL, 0);
	    else
#if 0
	      /* Roaming */
		...
		if(!strncmp(argv[2], "port", 4))
		  ...
		  goterr = set_private(0, argv + 1, argc - 1, "");
