		  ...
		  iwrange *	range)
{
  struct iwreq		wrq;
  char *buffer;
//  char			buffer[sizeof(iwrange)];	/* Large enough */
  int buffer_length;
  ...
  buffer_length = sizeof(iwrange);
  buffer = kmalloc(buffer_length,0);
  ...
  info.flags = 0;
  wrq.u.data.pointer = /*(caddr_t)*/ buffer;
  wrq.u.data.length = buffer_length;
  wrq.u.data.flags = 0;
    
  handler = rt28xx_iw_handler_def.standard[SIOCGIWRANGE - SIOCIWFIRST];
    
  if(handler(_pnet_device, &info, &wrq.u, buffer) < 0)
  {
    kfree(buffer);
    return(-1);
  }
...
//  iwprivargs *		newpriv;
   ...
   * sizes. Jean II */
      ...
      priv = kmalloc(maxpriv * sizeof(priv[0]),0);
      if(priv == NULL)
	{
...
//      priv = newpriv;
      ...
      ret = iw_handler_get_private(_pnet_device, &info, &wrq.u, (char *)priv);
//      ret = _pnet_device->do_ioctl(_pnet_device, &wrq, SIOCGIWPRIV);
...
//    ret = handler(_pnet_device, &info, &wrq.u, NULL);    
    ...
    if(ret == -E2BIG)
        fprintf(stderr, "%s: the buffer was too small\n", __FUNCTION__);
